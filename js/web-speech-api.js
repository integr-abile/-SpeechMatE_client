...
$(document).ready(function () {
  ...
  if (!("webkitSpeechRecognition" in window)) {
    ...
    $.get("../res/math_grammar.txt", function (data) {
      //carico il mio file di grammatica
      var speechRecognitionList = new webkitSpeechGrammarList();
      speechRecognitionList.addFromString(data, 1);
      recognition.grammars = speechRecognitionList;
    });

    $.get("../res/edit_grammar.txt", function (data) {
      //carico il mio file di grammatica
      var speechRecognitionList = new webkitSpeechGrammarList();
      speechRecognitionList.addFromString(data, 1);
    ...
    recognition.onresult = function (event) {
      ...
      for (var i = event.resultIndex; i < event.results.length; ++i) {
        if (event.results[i].isFinal) {
          ...
          final_transcript += spellingCorrection(
            event.results[i][0].transcript.toLowerCase()
          ); //0 indica l'alternativa più probabile. Decreasing confidence order
...
function postMathText(txt) {
  ...
  var addr = "http://" + env.HOST_IP + ":5001/mathtext";
...
function postEditText(txt) {
  var xhttp = new XMLHttpRequest();
  var addr = "http://" + env.HOST_IP + ":5001/editText";
  xhttp.open("POST", addr, true);
  xhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
  xhttp.send(JSON.stringify({ text: txt }));
}
...
function spellingCorrection(text) {
  if (text !== "") {
    ...
    for (var i = 0; i < structuredText.length; i++) {
      ...
      if (
        !structuredElem.protected &&
        structuredElem.token !== "" &&
        problematic_letters[structuredElem.token] !== undefined
      )
